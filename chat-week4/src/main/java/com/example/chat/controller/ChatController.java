...
public class ChatController {
    ...
    public void send(@Payload ChatMessage message) {
        ...
        if (message.getType() == null) {
            message.setType(ChatMessage.Type.TALK);
        }
    ...
    public void join(@Payload ChatMessage message) {
        ...
        roomUserService.join(message.getRoomId(), message.getSender());
    ...
    public void leave(@Payload ChatMessage message) {
        ...
        long now = System.currentTimeMillis();
        message.setTimestamp(now);
        message.setType(ChatMessage.Type.LEAVE);
        message.setContent(message.getSender() + " 님이 퇴장했습니다.");

        roomUserService.leave(message.getRoomId(), message.getSender());
        ...
        String destination = "/topic/room/" + message.getRoomId();
        log.debug("LEAVE from {} by {}", destination, message.getSender());
        messagingTemplate.convertAndSend(destination, message);
    }
